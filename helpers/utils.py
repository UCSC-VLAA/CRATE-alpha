...
def transform_weights(weights, num_blocks):
    transformed_weights = {}
    
    for i in range(num_blocks):
        # 构造原始权重键名
        out_key = f'Transformer/encoderblock_{i}/MultiHeadDotProductSubspaceAttention_0/out/kernel'
        proj_key = f'Transformer/encoderblock_{i}/MultiHeadDotProductSubspaceAttention_0/proj/kernel'
        
        # 构造新权重键名
        new_out_key = f'Transformer/encoderblock_{i}/Attention_0/out/kernel'
        new_proj_key = f'Transformer/encoderblock_{i}/Attention_0/proj/kernel'
        
        if out_key in weights:
            # 转换 out/kernel 权重
            out_kernel = weights[out_key]
            out_kernel_reshaped = out_kernel.transpose(0, 2, 1).reshape(768, 768)
            transformed_weights[new_out_key] = out_kernel_reshaped
        
        if proj_key in weights:
            # 转换 proj/kernel 权重
            proj_kernel = weights[proj_key]
            proj_kernel_reshaped = proj_kernel.reshape(768, 768)
            transformed_weights[new_proj_key] = proj_kernel_reshaped

    return transformed_weights

def load_checkpoint_debug(tree, npz):
  ...
  for k,v in  npz.items():
    ...
    # flag = True
    flag = False
    if 'MultiHeadDotProductSubspaceAttention_0' in k and flag:
      print(f"Before reshape {k}: {v.shape}")  # 打印重塑前的形状
      # if 'out' in k:
      #   v = v.transpose(2, 0, 1) 
      # if 'proj' in k:
      #   v = v.transpose(1, 2, 0) 
      v = jnp.reshape(v,(768, 768))

      print(f"After reshape {k}: {v.shape}")  # 打印重塑后的形状

      new_npz[k] = v
    else:
      new_npz[k] = v

  keys, values = zip(*list(new_npz.items()))
  
  '''
  Transformer/encoderblock_5/MultiHeadDotProductSubspaceAttention_0/out/kernel   | (12, 64, 768) 
  Transformer/encoderblock_5/MultiHeadDotProductSubspaceAttention_0/proj/kernel   (768, 12, 64) 

  Transformer/encoderblock_7/Attention_0/out/kernel      
| Transformer/encoderblock_7/Attention_0/proj/kernel     
  '''
...
def load_mae_weights(jax_vit_paras, model_variant='B/16', pretrained=False):
      ...
      if os.path.isfile(os.path.join(mae_eval_path, weight_name)):
         checkpoint = torch.load(os.path.join(mae_eval_path, weight_name), map_location='cpu')
      else:
         import torch.utils.model_zoo
         checkpoint = torch.utils.model_zoo.load_url(url, model_dir=str(mae_eval_path))
      ...
      def create_jax_tree(torch_state_dict, params_cpu):
        ...
        for k, v in torch_state_dict.items():
          ...
          if k == 'patch_embed.proj.weight':
            ...
            params['embedding']['kernel'] = jnp.array(v.numpy()).transpose(2, 3, 1, 0)#.reshape(shape)
          ...
          if 'blocks' in k:
            ...
            if 'norm1.weight' in k:
              params['Transformer'][new_block_name]['LayerNorm_0']['scale'] = jnp.array(v.numpy())
            if 'norm1.bias' in k:
              params['Transformer'][new_block_name]['LayerNorm_0']['bias'] = jnp.array(v.numpy())
            if 'norm2.weight' in k:
              params['Transformer'][new_block_name]['LayerNorm_1']['scale'] = jnp.array(v.numpy())
            if 'norm2.bias' in k:
              params['Transformer'][new_block_name]['LayerNorm_1']['bias'] = jnp.array(v.numpy())
            if 'mlp' in k:
              fc_num = int(k.split('.')[3][2]) - 1
              mlp_block_name = 'MlpBlock_0'
              fc_name = 'Dense_' + str(fc_num)
              if 'bias' in k:
                params['Transformer'][new_block_name][mlp_block_name][fc_name]['bias'] = jnp.array(v.numpy())
              else:
                #shape = params['Transformer'][new_block_name][mlp_block_name][fc_name]['kernel'].shape
                params['Transformer'][new_block_name][mlp_block_name][fc_name]['kernel'] = jnp.array(v.numpy()).transpose(1, 0)
            if 'attn' in k:
              attn_name = 'MultiHeadDotProductAttention_0'
              if 'proj' in k:
                if 'bias' in k:
                  params['Transformer'][new_block_name][attn_name]['out']['bias'] = jnp.array(v.numpy())
                else:
                  shape = params['Transformer'][new_block_name][attn_name]['out']['kernel'].shape
                  params['Transformer'][new_block_name][attn_name]['out']['kernel'] = jnp.array(v.numpy()).transpose(1, 0).reshape(
                    shape)
              else:
                ...
                if 'bias' in k:
                  ...
                  params['Transformer'][new_block_name][attn_name][qkv_list[0]]['bias'] = jnp.array(q_bias.numpy()).reshape(
                    shape)
                  params['Transformer'][new_block_name][attn_name][qkv_list[1]]['bias'] = jnp.array(k_bias.numpy()).reshape(
                    shape)
                  params['Transformer'][new_block_name][attn_name][qkv_list[2]]['bias'] = jnp.array(v_bias.numpy()).reshape(
                    shape)
                else:
...
def load_yutong_ckpt(jax_vit_paras, model='large'):
  ...
  if model == 'huge':
    ...
    width = 1024
  if os.path.isfile(os.path.join(mae_eval_path, weight_name + '.data-00000-of-00001')):
    # reader = ckpt.read(os.path.join(mae_eval_path, weight_name))
    import tensorflow as tf
    reader = tf.compat.v1.train.NewCheckpointReader(os.path.join(mae_eval_path, weight_name))
  ...
  params['pos_embedding'] = posemb_sincos_2d(16, 16, width=width, cls_token=True)
  for k, v in model_para.items():
    ...
    if '_encoder_layers' in k:
      ...
      if '_attention_layer_norm' in k and 'gamma' in k:
        params['Transformer'][new_block_name]['LayerNorm_0']['scale'] = jnp.array(v)
      if '_attention_layer_norm' in k and 'beta' in k:
        params['Transformer'][new_block_name]['LayerNorm_0']['bias'] = jnp.array(v)
      if '_output_layer_norm' in k and 'gamma' in k:
        params['Transformer'][new_block_name]['LayerNorm_1']['scale'] = jnp.array(v)
      if '_output_layer_norm' in k and 'beta' in k:
        params['Transformer'][new_block_name]['LayerNorm_1']['bias'] = jnp.array(v)
      if 'dense' in k and '_attention_layer' not in k:
        mlp_block_name = 'MlpBlock_0'
        if '_intermediate_dense' in k:
          fc_name = 'Dense_' + str(0)
          if 'bias' in k:
            params['Transformer'][new_block_name][mlp_block_name][fc_name]['bias'] = jnp.array(v)
          else:
            # shape = params['Transformer'][new_block_name][mlp_block_name][fc_name]['kernel'].shape
      ...
      if '_attention_layer' in k:
        ...
        if '_output_dense' in k:
          if 'bias' in k:
            params['Transformer'][new_block_name][attn_name]['out']['bias'] = jnp.array(v)
          else:
        ...
        if '_query_dense' in k:
          if 'bias' in k:
            params['Transformer'][new_block_name][attn_name]['query']['bias'] = jnp.array(v)
          else:
            params['Transformer'][new_block_name][attn_name]['query']['kernel'] = jnp.array(v)

        if '_key_dense' in k:
          if 'bias' in k:
            params['Transformer'][new_block_name][attn_name]['key']['bias'] = jnp.array(v)
          else:
            params['Transformer'][new_block_name][attn_name]['key']['kernel'] = jnp.array(v)

        if '_value_dense' in k:
          if 'bias' in k:
            params['Transformer'][new_block_name][attn_name]['value']['bias'] = jnp.array(v)
          else:
            params['Transformer'][new_block_name][attn_name]['value']['kernel'] = jnp.array(v)
