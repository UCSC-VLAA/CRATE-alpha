...
gcloud alpha compute tpus tpu-vm scp --recurse ~/CRATE-alpha/  $TPU_NAME:~/  --zone=$ZONE --worker=all --project ${PROJECT_ID}


#  prepare env will create an env installed all required softwares
gcloud alpha compute tpus tpu-vm ssh $TPU_NAME  --project=$PROJECT_ID --zone=$ZONE --worker=all --command "bash ~/CRATE-alpha/scripts/env/prepare_env.sh"