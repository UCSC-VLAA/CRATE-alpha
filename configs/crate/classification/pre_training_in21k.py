...
def get_config(arg=None):
  ...
  pp_common = (
      '|vgg_value_range'
      '|onehot(19167, key="{lbl}", key_result="labels")'
      '|keep("image", "labels")'
  )
  config.input.pp = (
      ...
      '|flip_lr'
      '|vgg_value_range'
      f'|onehot(19167, key="label", key_result="labels", on=1, off=0)'
      '|keep("image", "labels")'
  ...
  # Eval section
  def get_eval(split, dataset='imagenet21k'):
    return dict(
        type='classification',
