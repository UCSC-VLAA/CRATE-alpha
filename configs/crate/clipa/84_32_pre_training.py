...
def get_config(arg=None):
  ...
  arg = bvcc.parse_arg(
      arg,  res=84, runlocal=False, token_len=32, txt='bert_base', img='L/14',
  ...
  config.input = {}
  config.input.data = dict(name='datacomp1b', split='full', data_dir='The path of datacomp1b dataset')
  ...
  config.model.image_model = 'crate'
  ...
  dim = {'T': 192, 'S':384, 'B': 1024, 'L': 1024, 'H':1024}[arg.img[0]]
 # dim = 768
  ...
  config.masked_init = '[The path of the checkpoint for the text encoder]'
  config.masked_no_load = {'dont_load': ['.*img.*']}
  config.dont_load = {'dont_load': ['.*img/embedding/bias']}
