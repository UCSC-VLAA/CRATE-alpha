...
def main(argv):
    ...
    write_note(f"Are we conducting adv training: {config.get('adv.adv_train', False)}?")
    ...
    def update_fn(params, opt, rng, batch):
        ...
        if config.get("cpu_unit8", False) and not config.get('adv.adv_train', False):
            mean = jnp.asarray(
        ...
        if config.get('adv.adv_train', False):
            from adv import attack_clip as attack
            ...
            fgsm_attack = attack.UntargetedAttack(
                ...
                loss_fn=attack.untargeted_contrastive_loss)
